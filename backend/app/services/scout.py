...
def row_to_dict(row):
    return {"id": row["id"], "slug": row["slug"], "name": row["name"], "country": row["country"],
...
async def list_locations(pool, kind=None, lon=None, lat=None, radius_m=None):
    ...
    if kind and kind not in CONSUMER and kind != "both":
        clauses.append(f"(kind = ${idx} OR kind = 'both')"); args.append(kind); idx += 1
...
async def scout(pool, query, kind, lon, lat, radius_km, budget_usd, limit):
    ...
    from app.services.osm import cache_places, search_places
    parsed = await understand_query(query)
    effective = kind or parsed.get("kind") or "selfie"
    parsed["kind"] = effective
    live = await search_places(parsed.get("search_queries") or [query], lat=lat, lon=lon, limit=max(limit*2, 40))
    await cache_places(pool, live)
    local = []
    try:
        local = await list_locations(pool, kind=effective, lon=lon, lat=lat,
            radius_m=radius_km*1000 if lon is not None and lat is not None else None)
    except Exception:
        local = []
    by_slug = {}
    for item in local + live:
        by_slug[item.get("slug") or item["name"]] = item
    items = list(by_slug.values())
    for item in items:
        semantic = match_score(parsed.get("tags") or [], item["tags"], item.get("distance_m"))
...
async def investments(pool, query, lon, lat, radius_km, max_risk, limit):
    ...
    items = await list_locations(pool, kind=None, lon=lon, lat=lat,
        radius_m=radius_km*1000 if lon is not None and lat is not None else None)
    out = []
    for item in items:
